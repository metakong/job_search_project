...
### Phase 13.8: Safety-Net Fit Gate — closing the "unqualified roles in my Safety Net" leak (Opus 4.8)
- **Status**: Completed (validated: Node harness 9/9 against the real résumé; browser re-score wired via Dexie v7)
- **Context**: Owner reported still seeing roles he is **not qualified for inside the Safety Net** even after the 13.7 domain gate. Root-caused to `distributeAndRank()` in `scoring-coordinator.js`.
- **THE BUG**: the Safety Net was the **unconditional else-bucket**. Every other zone had an absolute fit gate (`STRIKE_FIT_MIN` 0.40, `MOONSHOT_FIT_MIN` 0.30) but the `dy <= -1 → 'safety'` branch (and the lateral / unknown-level fallbacks) assigned Safety with **no `delta_x` check at all** — the only floor anything cleared to land there was `NOISE_FIT_FLOOR` (0.18). The 13.7 domain gate correctly crushes off-field roles to `delta_x ≈ 0.00–0.25`, but because Safety *ignored* `delta_x`, those crushed roles **still fell into the Safety Net**. There was no `SAFETY_FIT_MIN` constant anywhere. This directly violated AGENTS.md Law 3 ("Safety Net … Delta-X > High Threshold … NOT a trash can for irrelevant jobs").
- **THE FIX (`scoring-coordinator.js`, `config.js`)**:
  - Added `SAFETY_FIT_MIN` and enforced it on **every** Safety-bound path (step-down, lateral, unknown-level, and the sub-Strike/sub-Moonshot fallbacks). A nominal step-down the candidate is not a real in-field match for is now hidden as `noise`, not filed under Safety.
  - Extended the **anti-blank-screen guard** to cover the new gate: if the entire relevant pool gets demoted to noise (a genuinely off-target batch, or a résumé whose fit never clears the bar), it is surfaced honestly in the Safety Net rather than rendering a "Zero Results" screen. Nothing weak ever reaches Strike/Moonshot.
- **Real-data calibration (not a guess)**: the Node harness calibrates on the owner's actual résumé and prints the real Delta-X distribution. The domain gate produces a **clean bimodal split** — genuine in-field roles score `delta_x ≈ 0.37–0.39`, off-field/unqualified roles are crushed to `≈ 0.00–0.02`. `SAFETY_FIT_MIN` was set to **0.30**, sitting in the empty valley between the two clusters: it keeps real in-field step-downs (entry sales rep 0.37 → Safety) while excluding every off-field role (lab tech 0.00, C++ eng 0.01 → noise). An initial 0.40 was rejected by the harness because it demoted a legitimate entry sales role — a real calibration finding, not a threshold picked from the air.
- **Observation for a follow-up (not fixed here)**: on this résumé even strong in-field matches top out at `delta_x ≈ 0.37–0.39`, just under the absolute `STRIKE_FIT_MIN` of 0.40, so the **Strike Zone is chronically starved** and near-laterals (RevOps Manager, +1 step) spill into Moonshot. This is a symmetric-overlap artifact of the current Delta-X (candidate→job *relevance*, 60-keyword denominator), **not** the reported bug. The O*NET pivot (job→candidate *qualification* coverage weighted by YoE) is the principled fix and will naturally re-separate Strike/Safety.
- **Config/SW/DB**: `VERSION` → `13.8.0`; `CACHE_NAME` → `job-search-v6`; Dexie **v7** forces a re-score so existing listings re-distribute through the Safety fit gate. Returning users may need a second hard reload (SW cache-first) to pick up the new JS.

### Phase 14.0 (SPIKE, not shipped): O*NET title→SOC resolvability — de-risking the pivot (Opus 4.8)
- **Status**: Spike complete — measured, documented, **no scoring changes made** (the point was to de-risk before committing).
- **What was built** (`tools/`): `build-onet.mjs` (minification pipeline: ingests `universal_classifiers/*.txt`, strips survey metadata, emits `pb_public/js/data/onet-zones.json` [SOC→Job Zone] and `onet-titles.json` [exact alias map + IDF token index]); `onet-resolver.mjs` (dependency-free, browser-portable normalizer + IDF-weighted token-vote resolver); `spike-accuracy.mjs` (accuracy harness over a 31-title REAL set: titles the owner actually saw + résumé roles + search queries + messy job-board formatting).
- **Artifact sizes**: `onet-zones.json` 2KB gz (trivial, useful). `onet-titles.json` 2.5MB raw / **450KB gz** (parses 66,512 aliases → 46,724 distinct normalized → 10,261 tokens). NOT ship-ready given the finding below; regenerate via `build-onet.mjs`.
- **RESULT — the linchpin is too weak**: free-text title→SOC scored **~61% raw accuracy** (always force best) / **76% precision but only 42% recall** (abstain-when-uncertain). The dangerous failures are **O*NET's own alias-table quirks returning CONFIDENT (`exact 1.00`) wrong occupations**: *Account Executive → Advertising & Promotions Managers*, *Product Manager → Marketing Managers*, *Customer Success Manager → Admin Support Supervisor* — abstain-gating can't catch exact matches, and they emit confidently-wrong **Job Zones** (a NEW seniority hallucination). Plus taxonomy lag on the owner's core modern roles (RevOps, AI Evaluator, CSM) and missing Job Zones on "All Other" catch-alls.
- **DECISION (architecture pivot)**: do **not** anchor Delta-Y or qualification on free-text title→SOC. Keep the dual-baseline title anchor (13.6) for seniority; leverage the already-working `competency-profiler` (13.7) to map job+résumé → ~15 domains, then map each domain to a **small curated set of representative SOCs** to pull O*NET's standardized skill/task/Job-Zone data. Qualification **coverage** (job→candidate — the true Safety-Net metric) can then be built from domain-anchored O*NET skill lists + résumé YoE, with **no fragile per-title resolution**. `onet-zones.json` is retained as an optional *supplementary* Delta-Y sanity signal, never a primary anchor.

### Phase 14.1 (SPIKE, not shipped): ESCO skill-layer extraction recall (Opus 4.8)
- **Status**: Spike complete — measured, documented, **no app changes** (build tooling only in `tools/`).
- **Question**: can LITERAL matching of the ESCO skill lexicon extract enough skills from real text to power a job→candidate qualification-coverage metric (the true Safety-Net definition)?
- **What was built** (`tools/`): `esco-extractor.mjs` (dependency-free, browser-portable n-gram surface-form matcher over all 99,233 ESCO surface forms incl. alt-labels) + `esco-spike.mjs` (recall + coverage measurement on the real résumé + representative in-/off-field JDs). Projected `esco-skills.json` = 3.26MB raw / **702KB gz**.
- **RESULT — literal extraction fails**: the real résumé (a rich 28-yr sales/ops/AI-eval executive) matched only **8 ESCO skills**, 6 of them single-token *noise* (history, psychology, logic, database). In-field JDs extracted **0–2** skills; **coverage did not separate** in-field from off-field. Root cause (not a bug — the matcher correctly catches verbatim phrases like "infection control", "agile development"): **96% of ESCO skills are multi-word competence *phrases*** ("manage sales pipeline") that never appear verbatim in natural résumé/JD prose ("owned the pipeline", "closed $2M in new logos"). Multi-word literal → near-zero recall; single-token → noise.
- **DECISION**: **do not adopt ESCO via literal/lexical matching** — it is measurably worse than the status quo. The existing curated `competency-profiler` (13.7) already produces clean bimodal separation (in-field Δx≈0.37 vs off-field≈0.00) and **outperforms ESCO-literal**. ESCO's granularity only pays off with a **semantic bridge** (embed the ~14K ESCO skill phrases + résumé/JD sentences, cosine-match) — and that must be *measured to beat the curated profiler* before adoption, not assumed. This is the data-backed case for the semantic/LLM extraction layer captured in `BRAINSTORMING.md`. Next candidate step (if pursued): feasibility-measure embedding 14K ESCO phrases in-browser via the already-shipped transformers.js `all-MiniLM` local embedder (keyless, private).

### Phase 14.2 (SPIKE, validated): YoE / "Highest & Best Use" profiler (Opus 4.8)
- **Status**: Spike validated on the real résumé — **no app changes yet** (`tools/yoe-spike.mjs`). Chosen direction after both taxonomy spikes came back negative: build the YoE profiler on the **curated vocabulary** (competency-profiler domains + soft lexicon), not ESCO/O*NET.
- **Approach**: slice the résumé into dated roles (reusing the date-anchor logic), attribute each role's DURATION to the curated skills demonstrated in that role's window, accumulate years per skill → top-5 hard + top-5 soft = "best & highest use."
- **Honesty fixes found & applied during the spike** (v1 was misleading; v2 is credible):
  - **Calendar-union years** (merge overlapping role intervals per skill) — kills overlap double-counting (v1 ranked "logistics" 20.9y #1 by summing the self-employed role's calendar time twice).
  - **Prominence gate** (attribute a skill to a role only if it's a concrete *signal* term, OR appears ≥2×, OR is in the title) — kills incidental-mention inflation (v1 invented **16.6y "finance"** from a single "payroll" mention in an 11.8y role).
  - **Stemmed + expanded soft matcher** — v1 found ZERO soft skills (they live in the summary header + morphology mismatches: "mentor"≠"mentoring", "renegotiations"≠"negotiation").
- **Validated result (real résumé)**: 8 roles parsed with correct durations; domains = operations 15.4y / sales 14.8y / automation 3.4y / ai_eval 1.5y (off-field domains correctly absent); TOP HARD = logistics, salesforce, procurement, operations, SOPs; TOP SOFT = leadership 19.7y, negotiation, strategy, teamwork, organization. A faithful sales/ops-exec signature matching the recruiter-doc.
- **Known refinement for the real implementation**: the 11.8y self-employed role still lets a single-mention *signal* skill inherit the full 11.8y (salesforce/procurement/logistics cluster there). Add a **self-employment duration discount** (same recruiter-discount principle as the dual-baseline seniority anchor) when wiring into the app.
- **Next**: wire the validated logic into `pb_public/js/` (new `yoe-profiler.js` + `resume-parser.js` calibration/persistence), surface a "Highest & Best Use" panel for the user to sanity-check, and later feed YoE into qualification-weighted zoning. (Pending owner go-ahead — this is the first app-code step of the effort.)

### Phase 14.3: YoE / "Highest & Best Use" profiler — Increment 1 SHIPPED (Opus 4.8)
- **Status**: Shipped to `pb_public/js/`. Validated: Node assertion harness 11/11 (`tools/yoe-verify.mjs`) + live browser (panel renders, correct data/layout, no console errors).
- **Scope (display-first, deliberately NOT yet in the scoring loop)** — lets the user sanity-check the signature before it can influence zoning:
  - **NEW `pb_public/js/scoring/yoe-profiler.js`** (`window.yoeProfiler.computeProfile(resumeText)`): slices the résumé into dated roles, attributes each role's duration to the curated skills demonstrated in it, returns top-10 hard + top-10 soft + domains by cumulative years. Ports the validated spike logic incl. the three honesty guards (prominence gate, weighted calendar-union, self-employment 0.6× discount) and a **tight self-employment detector that excludes bare "LLC"** (the spike false-flagged "AP Wireless … LLC" — real employers are LLCs too).
  - **`resume-parser.js`** `calibrateFromText` now also returns `yoeProfile`; persisted everywhere calibration is saved (`setup-wizard.js` submit, `resume-parser.saveResumeText`, `app.js ensureCalibration`).
  - **`app.js`**: `CALIBRATION_VERSION` **3 → 4** (existing résumé users auto-recompute on next load); new `renderBestUse(profile)` renders a **"🎓 Your Highest & Best Use"** panel (top-5 hard + top-5 soft with year bars) between the stats and controls; hidden entirely when there's no résumé.
  - **`index.html`** (panel markup + `yoe-profiler.js` script tag after `competency-profiler.js`), **`sw.js`** (`CACHE_NAME` v6 → **v7**, `yoe-profiler.js` added to precache), **`style.css`** (`.best-use-*` styles, theme-matched, responsive 2-col→1-col).
- **Validated result (real résumé)**: 8 roles; HARD = logistics 12.9y, operations 9y, salesforce 7.1y (discounted from 11.8 raw — self-emp), procurement 7.1y, SOPs 4.8y; SOFT = leadership 17.1y, negotiation 7.6y, strategy, teamwork, organization; domains operations/sales only (off-field + phantom-finance correctly absent). A faithful sales/ops-exec signature.
- **Known minor nit (deferred)**: near-duplicate curated terms (`sop`/`sops`) can appear as two rows; only one reaches the top-5 for this résumé, but a dedup pass would polish it.
- **Increment 2 (later, not started)**: feed YoE into qualification-weighted zoning (Safety = candidate exceeds the years the role implies) and address the Strike-Zone starvation.

### Docs: added `BRAINSTORMING.md` (2026-07-01)
- Created **`BRAINSTORMING.md`** as a non-binding living scratchpad for ideas / potential next steps / new data sources that *may or may not* ship. Nothing there is a commitment; graduated ideas move here + into code, rejected ones stay with their rationale. Seeded with the 2026-07-01 external-data-source evaluation (ESCO / Lightcast / Wikidata / spaCy / embeddings) — headline: **adopt ESCO's *skill layer* to build honest job→candidate qualification coverage**, reject all server/runtime-API/Python options as North-Star violations. **Next agreed step (pending owner's ESCO download):** spike the ESCO skill lexicon the same way O*NET was spiked (measure before building).
- Referenced from `AGENTS.md` (Documentation Synchronization) and `README.md` (Project Structure + Contributing).
