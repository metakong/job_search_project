...
*   **The Domain-Competency Gate (NON-NEGOTIABLE):** Flat keyword overlap alone is a lie — a sales/ops résumé full of AI/systems *buzzwords* ("systems", "AI", "red team", "pipeline", "architecture") will false-match software-engineering / data-science / clinical postings at 80–100%. Delta-X MUST be multiplied by a **domain-compatibility multiplier** from `competency-profiler.js`: it classifies the résumé and the job into weighted skill DOMAINS using **concrete, discriminative terms only** (named tools/languages/certs — never generic buzzwords), and damps fit when the job's domain is one the candidate has little résumé-derived standing in (hard-capped when the job's *primary* domain has candidate affinity < 0.2). The **résumé is ground truth** for the candidate's domain — the search categories the user ticked do NOT open the fit gate. When adding domain terms, keep collision-prone words in a single domain (`red team` → security, not ai_eval; `coding` is NOT software_eng). The gate is non-fatal (toxicity/Core/trajectory still compute) and skips when there is no résumé/job domain signal (so it never blanks the screen).

### Law 2: Delta-Y (The Trajectory) & Anti-Hallucination
Delta-Y measures the step-up or step-down in seniority (`Job Seniority - Candidate Effective Baseline`).
*   **The Dual-Baseline Anchor (NON-NEGOTIABLE):** The candidate baseline for Delta-Y is the **effective baseline = `Math.floor((peakSeniority + recentSeniority) / 2)`**, NOT the peak alone. A candidate with a high peak but a low recent level (an underemployed senior — the exact persona this tool serves) is realistically re-hired *between* the two. Anchoring to the peak alone makes every real job a "step down," turns the Safety Net into a trash can, and leaves Moonshot mathematically empty. Routing (`distributeAndRank`) and friction MUST use `trajectory_effective`. For a linear career (`peak == recent`) this collapses to a single level, so nothing regresses for normal candidates.
*   **Recent-Level Detection:** `resume-parser.js` derives `recentSeniority` from the most-recent role's **title window, bounded by the previous date anchor** (never a fixed char look-back — it bleeds across roles), and **skips self-employment / founder / 1099-contract titles** (inflated, recruiter-discounted). The wizard's "Realistic Current Level" dropdown is the user-facing authority; the parser only pre-fills it.
*   **Title-Strict Parsing:** Job seniority extraction (`detectSeniority`) MUST be restricted exclusively to the `job.title` string. **DO NOT** scan `description_full` for seniority keywords, as this causes reporting-structure hallucinations (e.g., "reports to the Director" flagging an Entry role as a Director).
