    ...
    const dbAdapter = {
        ...
        async getJobs(filters = {}, page = 1, perPage = 50, preloaded = null) {
            try {
                ...
                if (filters.strategy_tier) {
                    items = items.filter(it => it.strategy_tier === filters.strategy_tier);
                }
            ...
            try {
                ...
                if (toInsert.length) {
                    ...
                    for (let i = 0; i < toInsert.length; i += chunkSize) {
                        ...
                        await window.localDB.job_listings.bulkPut(chunk);
                    }
                }
        ...
        async persistJobs(jobsList) {
            try {
                const chunkSize = window.CONFIG?.SCORING_CHUNK_SIZE || 25;
                for (let i = 0; i < jobsList.length; i += chunkSize) {
                    await window.localDB.job_listings.bulkPut(jobsList.slice(i, i + chunkSize));
                }
                return true;
            } catch (err) {
                console.error('[DB Adapter] Failed to persist jobs:', err);
                return false;
            }
        },
        
        async bulkUpdate(jobsList) {
            return this.persistJobs(jobsList);
        },
